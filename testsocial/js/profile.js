$(document).ready(function() {
    ...
    $('#save_thumb').click(function() {
        ...
        if (x1 == "" || y1 == "" || x2 == "" || y2 == "" || w == "" || h == "") {
            ...
            $.ajax({
                type: 'POST',
                url: config.baseurljs + "crop_script.php",
                data: "filename=" + $('#filename').val() + "&x1=" + x1 + "&x2=" + x2 + "&y1=" + y1 + "&y2=" + y2 + "&w=" + w + "&h=" + h,
                success: function(data) {
                    thumb.attr('src', config.baseurljs + 'uploads/thumb_' + $('#filename').val());
...
function createUploader() {
    // alert(config.baseurljs);
    var button = $('#upload');
    var uploader = new qq.FileUploaderBasic({
        button: document.getElementById('file-uploader'),
        action: config.baseurljs + 'upload',
...
function load_original(filename) {
    ...
    $('#thumbnail').attr('src', config.baseurljs + "uploads/" + filename);
    $('#thumb_preview').attr('src', config.baseurljs + "uploads/" + filename);
