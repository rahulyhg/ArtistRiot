...
class Ion_auth_model extends CI_Model
{
	...
	public function activate($id, $code = false)
	{
		...
		if ($code !== FALSE)
		{
			$query = $this->db->select($this->identity_column . ',id, email, first_name, last_name, profile_created,role')
			...
			$session_data = array(
					...
					'user_id'                   => $user->id,
					'email'                => $user->email,
					'first_name'           => $user->first_name, //everyone likes to overwrite id so we'll use user_id
	...
	public function registerFacebookUser($username, $password, $email, $additional_data = array(), $groups = array())
	{
		$this->trigger_events('pre_register');
		...
		if ($this->identity_column != 'username')
		{
			...
			$ip_address = $this->_prepare_ip($this->input->ip_address());
			$salt       = $this->store_salt ? $this->salt() : FALSE;
			$password   = $this->hash_password($this->salt(), $salt);
	 ...
	 * @author Mathew
	 **/
	public function login($identity, $password, $role, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', username, email, id, password, active, first_name, last_name, last_login, profile_created, role')
		                  ->where(array($this->identity_column => $identity, 'role' => $role))
		                  ->limit(1)
		                  ->get($this->tables['users']);
	...
	/**
	 * Facebook login method. This method will login the user without taking the user's password. 
	 * 
	 * 
	 */

	public function facebookUserLogin($identity, $remember=FALSE)
	{
		$this->trigger_events('pre_login');
		...
		$query = $this->db->select($this->identity_column . ', username, first_name, last_name, email, id, password, active, last_login, profile_created, role')
		->where($this->identity_column, $identity)
		->limit(1)
		->get($this->tables['users']);
	...
	**/
	...
	public function set_session($user)
	{
		...
		$session_data = array(
		    'user_id'             => $user->{$this->identity_column},
	...
	**/
	...
	// Code to insert captcha
	
	public function insertCaptcha($data){
		$query = $this->db->insert_string('ar_captcha', $data);
